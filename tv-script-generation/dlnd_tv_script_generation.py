...
def get_init_cell(batch_size, rnn_size):
    ...
    cell = tf.contrib.rnn.MultiRNNCell([make_cell(rnn_size) for _ in range(1)])
...
def build_nn(cell, rnn_size, input_data, vocab_size, embed_dim):
    ...
    output = tf.contrib.layers.fully_connected(rnn, vocab_size, activation_fn=None)
...
def get_batches(int_text, batch_size, seq_length):
    ...
    text_target = np.zeros_like(int_text)
    text_target[:-1], text_target[-1] = int_text[1:], int_text[0]
    
    int_text = int_text[:n_batches * characters_per_batch]
    int_text = int_text.reshape((batch_size, -1))
    
    text_target = text_target[:n_batches * characters_per_batch]
    text_target = text_target.reshape((batch_size, -1))
    
    temp = np.array(0)
    for i in range(0, int_text.shape[1], seq_length):
        x = int_text[:,i:i+seq_length]
        x = x.reshape(-1)
        y = text_target[:,i:i+seq_length]
        y = y.reshape(-1)
        temp = np.append(temp, x)
        temp = np.append(temp, y)
        
    
    temp = temp[1:].reshape((n_batches, 2, batch_size, seq_length))
    return temp
